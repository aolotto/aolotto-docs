...
1. 不可预测性：Arweave的区块Hash是基于该区块全部交易内容生成的唯一性ID，轮次结束后的第五个区块的hash值，在用户下注时完全无法预测。
2. 可验证性：issac cipher的随机数函数在种子确定的情况下生成的结果也是确定的，所以在种子不变的的情况下，意味着结果可被验证，这可以有效避免计算节点(CU)在算法执行中进行黑箱操作的可能性；
3. 取值分布的随机性：3位数号码采用循环单位
